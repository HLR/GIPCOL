...
def load_glove_embeddings(vocab):
    ...
    emb_file = os.path.join("/localscratch2/xugy/Data/glove", 'glove.6B.300d.txt')
...
def get_pair_scores_objs(target_attr, target_obj, all_objs, dict_Obj2AttrList, obj_embedding_sim):
    score = -1.
    for o in all_objs:
        if o!=target_obj and target_attr in dict_Obj2AttrList[o]:
            temp_score = obj_embedding_sim[(target_obj, o)]
            if temp_score>score:
                score=temp_score
    return score

def get_pair_scores_attrs(attr, obj, all_attrs, dict_Attr2ObjList, attr_embedding_sim):
    score = -1.
    for a in all_attrs:
        if a != attr and obj in dict_Attr2ObjList[a]:
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(dataset_path, f'feasibility_{config.dataset}.pt')
    if os.path.exists(save_path):
        # -----------------------
        # analysze scores
        # -----------------------
        feasibility = torch.load(save_path)['feasibility']
