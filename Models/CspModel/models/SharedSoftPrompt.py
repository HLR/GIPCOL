...
class SharedSoftPromptInterface(CLIPInterface):
    def __init__(
        self,
        clip_model,
        config,
        ...
        shared_soft_prompt_emb,     # prompt
        pair_txt_input_ids,         # input = [prompt, cls_label]
        attr_txt_input_ids,
        obj_txt_input_ids,
        device="cuda",
        enable_pos_emb=True,
        ...
        self.shared_soft_prompt_emb = shared_soft_prompt_emb
    ...
    def construct_pair_txt_emb(self, pair_attr_obj_idx):
        ...
        pair_txt_token_emb[:, 1 : len(self.shared_soft_prompt_emb) + 1, :] = self.shared_soft_prompt_emb.type(self.clip_model.dtype)
    ...
    def construct_attr_txt_emb(self, attr_idx):
        ...
        # step 3: replace soft prompt
        attr_txt_token_emb[:, 1: len(self.shared_soft_prompt_emb) + 1, :] = self.shared_soft_prompt_emb.type(self.clip_model.dtype)
    ...
    def construct_obj_txt_emb(self, obj_idx):
        ...
        # step 3: replace soft prompt
        obj_txt_token_emb[:, 1: len(self.shared_soft_prompt_emb) + 1, :] = self.shared_soft_prompt_emb.type(self.clip_model.dtype)

        return obj_txt_token_emb


    def set_soft_embeddings(self, shared_soft_prompt):
        """ """

        if shared_soft_prompt.shape == self.shared_soft_prompt_emb.shape:
            self.state_dict()['shared_soft_prompt_emb'].copy_(shared_soft_prompt)
        else:
            raise RuntimeError(
...
def shared_soft_prompt_init(
    ...
    shared_soft_prompt_embedding = nn.Parameter(ctx_vectors).to(device)
...
def get_shared_soft_prompt(train_dataset, config, device):
    """   """
    # clip model, prompt and element
    (
        clip_model,
        soft_ele_embedding,
        shared_prompt_embedding,
        pair_txt_input_ids,
        attr_txt_input_ids,
        obj_txt_input_ids,
        offset
    ) = shared_soft_prompt_init(train_dataset, config, device)
    ...
    interface = SharedSoftPromptInterface(
        clip_model,
        config,
        offset,
        soft_ele_embedding,
