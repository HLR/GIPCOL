...
class Evaluator:
    ...
    def __init__(self, dset, model, fast=False):
        ...
        pairs = [(dset.attr2idx[attr], dset.obj2idx[obj]) for attr, obj in dset.full_pairs]
        self.train_pairs = [(dset.attr2idx[attr], dset.obj2idx[obj]) for attr, obj in dset.train_pair_id_list]
        ...
        if dset.phase == 'train':
            ...
            test_pair_set = set(dset.val_pairs + dset.train_pair_id_list)  # todo: check if this makes sense
            test_pair_gt = set(dset.val_pairs)
        else:
            print('Evaluating with test pairs')
            test_pair_set = set(dset.test_pairs + dset.train_pair_id_list)
            test_pair_gt = set(dset.test_pairs)

        if dset.open_world and not fast:
            masks = [1 for _ in dset.full_pairs]
        else:
            masks = [1 if pair in test_pair_set else 0 for pair in dset.full_pairs]

        self.closed_mask = torch.BoolTensor(masks)
        # Mask of seen concepts
        seen_pair_set = set(dset.train_pair_id_list)
        mask = [1 if pair in seen_pair_set else 0 for pair in dset.full_pairs]
        self.seen_mask = torch.BoolTensor(mask)

        # Object specific mask over which pairs occur in the object oracle setting
        oracle_obj_mask = []
        for _obj in dset.objs:
            mask = [1 if _obj == obj else 0 for attr, obj in dset.full_pairs]
    ...
    def score_manifold_model(self, scores, obj_truth, bias=0.0, topk=5):
        ...
        scores = torch.stack(
            [scores[(attr, obj)] for attr, obj in self.dset.full_pairs], 1
        )  # (Batch, #pairs)
        orig_scores = scores.clone()
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk=1,std_bias=1e3):
        # Go to CPU
        ...
        base_scores = torch.stack(
            [allpred[(attr, obj)] for attr, obj in self.dset.full_pairs], 1
        )  # (Batch, #pairs)
        ...
        for bias in biaslist:
            ...
            att_accuracy.append(float(results[0].mean()))
            obj_accuracy.append(float(results[1].mean()))
        ...
        #stats['hm_attr'] = att_accuracy[idx]
        #stats['hm_obj'] = obj_accuracy[idx]
